...
// Componente de Tarjeta Unificado (Versión Dark)
...
/* ========================================================================
   PÁGINA PRINCIPAL - CONFLICTO RESUELTO (VERSIÓN DARK)
   ======================================================================== */
const Freelancers = () => {
    // --- Estados de Datos ---
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-100 font-sans pb-20">
            ...
            {/* ===== HERO SEARCH ===== */}
            <div className="bg-slate-900 sticky top-0 z-30 shadow-lg shadow-black/30">
                        ...
                        <div className="flex gap-2 w-full lg:w-auto">
                            <div className="relative w-full lg:w-96 group">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-4 py-2.5 bg-slate-800 text-white placeholder-slate-500 focus:bg-slate-700 focus:ring-2 focus:ring-blue-500/50 rounded-xl outline-none transition-all"
                                    value={searchTerm}
                            ...
                            <button
                                ...
                                className="lg:hidden p-2.5 bg-slate-800 rounded-xl text-slate-300 hover:bg-slate-700"
                            >
                                <Filter size={20} />
                            </button>
                        </div>
            ...
            <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    `}>
                        ...
                        <div className="bg-slate-900 p-5 rounded-xl shadow-md h-fit lg:sticky lg:top-28 space-y-8">
                            {/* Header Desktop */}
                            <div className="hidden lg:flex items-center gap-2 mb-5 pb-3">
                                ...
                                <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
                                    {especialidadesList.map(esp => (
                                        <label key={esp} className="flex items-center gap-3 cursor-pointer group p-1 rounded hover:bg-slate-800/50">
                                            <div className={`w-4 h-4 rounded flex items-center justify-center transition-all ${filterEspecialidad === esp ? 'bg-blue-600' : 'bg-slate-800'}`}>
                                ...
                                <div className="flex flex-col gap-1">
                                    {[5, 4, 3, 2, 1].map(stars => (
                                        <button
                                            ...
                                            className={`flex items-center justify-between px-3 py-2 rounded-lg text-sm transition-colors w-full ${filterRating === stars ? 'bg-slate-800 text-yellow-500' : 'hover:bg-slate-800 text-slate-400'}`}
                                        >
                                            <div className="flex text-yellow-400">
                                                {[...Array(5)].map((_, i) => (
                                                    <Star key={i} size={14} className={i < stars ? "fill-current" : "text-slate-800"} />
                                                ))}
                                            </div>
                                        </button>
                                    ))}
                        ...
                        {showMobileFilters && (
                            <div className="lg:hidden sticky bottom-0 left-0 right-0 p-4 bg-slate-900 mt-auto shadow-inner">
                                <button
                    ...
                    <div className="flex-1">
                        <div className="mb-6 flex justify-between items-center">
                            <p className="text-slate-400 text-sm">
                                Se encontraron <span className="font-bold text-white">{generalData.length}</span> resultados
                            </p>
                            ...
                            {generalData.length > 0 && (
                                <div className="flex items-center gap-2">
                                    ...
                                    <button
                                        ...
                                        className="p-1.5 rounded-md bg-slate-900 hover:bg-slate-800 disabled:opacity-30 text-slate-400 transition-colors"
                                    >
                                        <ChevronLeft size={16} />
                                    </button>
                                    <button
                        ...
                        {paginatedGeneralData.length > 0 ? (
                            ...
                            <div className="text-center py-24 bg-slate-900 rounded-2xl">
                        ...
                        {totalPages > 1 && paginatedGeneralData.length > 0 && (
                            <div className="mt-12 flex justify-center">
                                <div className="flex items-center gap-2 bg-slate-900 p-2 rounded-xl shadow-lg shadow-black/20">
                                    <button
                                        onClick={() => handlePageChange(currentPage - 1)}
                                        disabled={currentPage === 1}
